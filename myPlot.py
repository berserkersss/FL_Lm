...
if __name__ == '__main__':
    ...
    for i in range(num):
        ...
        for j in range(data_num):
            csv_path_accuracy = 'result/SVM' + str(j) + '/Accuracy_' + labels[i] + '_SVM.csv'
            accuracy = pd.read_csv(csv_path_accuracy, header=None)
            accuracy = accuracy.values
            accuracy = np.delete(accuracy, -1, axis=1)
            A_Len = len(accuracy[0])
            index = np.argmax(accuracy)
            R_array = accuracy[0][index - 10:index]
            A_array = np.tile(R_array, (1, int(A_Len / 10 - index / 10) + 1))
            accuracy = np.delete(accuracy, index + np.arange(A_Len - index), axis=1)
            accuracy = np.hstack((accuracy, A_array))
            accuracy = np.delete(accuracy,  A_Len + np.arange(len(accuracy[0])-A_Len), axis=1)
            if j == 0:
                accuracy_value = accuracy/3
            else:
                accuracy_value = accuracy_value + accuracy/3
        ax.plot(accuracy_value[0], c=colors[i], label=labels[i], linestyle=linestyles[i])
        ax.legend()
        plt.xlabel('Iterations')
        plt.ylabel('Accuracy')
